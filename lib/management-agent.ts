...
async function managementRequest(path: string, init: RequestInit) {
  ...
  if (!gatewayUrl && !tunnel) throw new Error("Der Management-Agent ist noch nicht mit der Website verbunden.");
...
export async function forwardToManagementAgent(payload: unknown) {
  return managementRequest("/jobs", { method: "POST", body: JSON.stringify(payload) });
}

export async function readManagementAgentResult(jobId: string) {
  return managementRequest(`/results?jobId=${encodeURIComponent(jobId)}`, { method: "GET" });
}
