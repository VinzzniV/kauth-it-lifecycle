  ...
  try {
    const payload = z
      .object({
        ...
        referenceUser: z
          ...
          .regex(
            /^[\p{L}\p{N} ._'@-]+$/u,
            "Der Referenzbenutzer enthält ungültige Zeichen.",
          ),
        adCredential: z.object({
          username: z
            .string()
            .trim()
            .min(1)
    ...
    const job = {
      ...
      directory: {
        domain: "kauth.local",
        referenceUser: {
          query: payload.referenceUser,
          displayName: payload.referenceUser,
        },
      },
      adCredential: payload.adCredential,
      automationTaskIds: [],
    };
    ...
    await db
      .update(employees)
      .set({
        directoryReferenceUser: payload.referenceUser,
        directoryReferenceStatus: "checking",
        directoryReferenceMessage: "Prüfung läuft",
        directoryTargetOu: "",
        updatedAt: now,
      })
    ...
    try {
      ...
      await db
        ...
        .where(eq(automationRuns.id, jobId));
      await db
        .update(employees)
        .set({
          directoryReferenceStatus: "error",
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Referenzbenutzer konnte nicht geprüft werden.";
    return Response.json(
      { error: message },
